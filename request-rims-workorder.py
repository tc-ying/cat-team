""" Login FLBWMASS, sniff desktop Chrome's cookie, and request RIMS Order JSONs """
...
def main(argv):
    ...
    for estate in argv:
        ...
        form = {'pageProgramTypeCode': 'P', 'woCreDateFrom': '01072020', 'woCreDateTo': '30062023', 'hseEstKey': hseEstKey, 'woTypeCode': 'C', 'mntSchmCode': '', 'max': '14000', 'page': '1', 'sort': '', 'order': 'asc'}
        response = session.post(url, params=params, data=form, verify=False, timeout=480)
        print("Request Status: ", response.status_code)

        with open('./__EWO Handling (TMSS)/' + flCode + '.json', 'wb') as f:
