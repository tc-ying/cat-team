""" Login FLBWMASS, sniff desktop Chrome's cookie, and request VFR Order JSONs """
...
def main(argv):
    ...
    url = 'https://tomctms.int.housingauthority.gov.hk/ctms/vfrworksorderenquiry.rpc'
    ...
    for estate in argv:
        ...
        form = {'vfrWoCretDateFrom': '01072020', 'vfrWoCretDateTo': '30062023', 'hseEstKey': hseEstKey, 'max': '2500', 'page': '1', 'sort': '', 'order': 'asc'}
        response = session.post(url, params=params, data=form, verify=False, timeout=300)
        print("Request Status: ", response.status_code)

        with open('./__VFR Handling (TMSS)/' + flCode + '.json', 'wb') as f:
